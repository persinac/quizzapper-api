...
export class UpdatedView1595452981969 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query("DELETE FROM `quizapp`.`typeorm_metadata` WHERE `type` = 'VIEW' AND `schema` = ? AND `name` = ?", ["quizapp","TestAttemptDetail_View"]);
        ...
        await queryRunner.query("INSERT INTO `quizapp`.`typeorm_metadata`(`type`, `schema`, `name`, `value`) VALUES (?, ?, ?, ?)", ["VIEW","quizapp","TestAttemptDetail_View","select concat(ts.testSummaryID, '-',ta.testAttemptID,'-',tr.responseID) as id\n        , ts.testSummaryID, ts.withinTimeLimit, ta.testAttemptID, tr.responseID, tr.response, q.questionID\n        , q.answers, q.correctAnswer, q.difficulty, q.question\n        from test_summary ts\n        JOIN test_attempt ta ON ts.testAttemptID = ta.testAttemptID\n        JOIN test_response tr ON ta.testAttemptID = tr.testAttemptID\n        JOIN question q ON tr.questionID = q.questionID"]);
    }
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        "JOIN test_response tr ON ta.testAttemptID = tr.testAttemptID " +
        "JOIN question q ON tr.questionID = q.questionID");
        await queryRunner.query("INSERT INTO `quizapp`.`typeorm_metadata`(`type`, `schema`, `name`, `value`) VALUES (?, ?, ?, ?)", ["VIEW","quizapp","TestAttemptDetail_View","select ts.testSummaryID, ts.withinTimeLimit, ta.testAttemptID, tr.responseID, tr.response, q.questionID\n        , q.answers, q.correctAnswer, q.difficulty, q.question\n        from test_summary ts\n        JOIN test_attempt ta ON ts.testAttemptID = ta.testAttemptID\n        JOIN test_response tr ON ta.testAttemptID = tr.testAttemptID\n        JOIN question q ON tr.questionID = q.questionID"]);
    }
